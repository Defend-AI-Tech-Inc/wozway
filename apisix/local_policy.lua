...
local PATTERNS = {
    ...
    -- AWS Access Keys (AKIA followed by exactly 16 uppercase letters/digits)
    -- Note: Lua patterns have no {n} quantifier, so each char is listed explicitly
    aws_key = {
        pattern = "AKIA[%u%d][%u%d][%u%d][%u%d][%u%d][%u%d][%u%d][%u%d][%u%d][%u%d][%u%d][%u%d][%u%d][%u%d][%u%d][%u%d]",
        name = "AWS Access Key",
        action = "BLOCK"
    },
    -- Private keys
